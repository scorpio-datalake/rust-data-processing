...
from tests.pipeline_fixture_support import load_schema_fields, pipeline_transform_sql, resolve_pipeline_json
...
def test_jvm_contract_dataframe_centric_pipeline_template_resolves() -> None:
    payload = json.loads(
        resolve_pipeline_json(
            ...
            {
                "SOURCE_PATH": str(
                    fixture_path("jvm_contract_three_rows.json")
                ),
...
def test_dataframe_centric_sql_filter_and_multiply_matches_doc_example() -> None:
    ...
    sql = pipeline_transform_sql(
        "jvm_contract", "pipelines/dataframe_centric_sql.pipeline.json"
    )
