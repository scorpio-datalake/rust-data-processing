...
Accepted — implements **`Planning/PHASE3_EPICS.md`** **P3-E1-S9** (orchestration contract) and frames **P3-E1-S10** (lake / relational sinks).
...
8. **DB sources:** declare ConnectorX URLs + SQL in **`sources.db_reads`** (`postgresql://`, `oracle://`, `mssql://`, `mysql://`). `jdbc:` schemes are rejected at parse. Requires **`rdp_jvm_sys`** built with **`db_connectorx`**; results in **`db_source_results`**. Without that feature, ingest fails with **`DB_CONNECTORX_NOT_BUILT`**. Otherwise export query results to a **local** file and use **`sources.paths`**.
