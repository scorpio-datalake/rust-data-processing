...
2. **docs.rs** — updates automatically for the published Rust version. If a version shows **“failed to build”**, open **Builds** on docs.rs. Do **not** set `cargo-args = ["-j", "1"]`: docs.rs already passes `-j`, and a second `--jobs` makes Cargo exit immediately (`--jobs` cannot be used multiple times; this failed **0.3.6**). Publish a **patch** after a metadata or rustdoc fix so docs.rs rebuilds. For **compiler errors**, fix the reported issue and republish.
