...
Java examples page: CI installs **Pandoc**, runs `pandoc docs/java/EXAMPLES.md -o site/java/examples.html` (see `.github/workflows/docs.yml`) with a small header stylesheet under `docs/landing/java-examples-pandoc-header.html`, copies **`docs/java/examples/*.java`** to **`site/java/examples/`**, and runs **`scripts/python_scripts/java_examples_pages_links.py`** to rewrite relative `.md` / repo paths to GitHub blob URLs (or `../python/examples.html` on Pages) while keeping **`examples/*.java`** local.
