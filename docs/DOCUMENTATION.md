...
Java examples page: CI installs **Pandoc**, runs `pandoc docs/java/EXAMPLES.md -o site/java/examples.html` (see `.github/workflows/docs.yml`) with a small header stylesheet under `docs/landing/java-examples-pandoc-header.html`, copies **`docs/java/examples/*.java`** to **`site/java/examples/`**, and runs **`java_examples_pages_links.py`** so repo-external paths become GitHub blob URLs while **`examples/*.java`** and shared docs like **`../CONNECTORS.html`** stay on Pages. **`copy_pages_markdown.py --html`** renders shared cross-linked docs (`CONNECTORS.html`, `CLOUD_AUTH.html`, …) with the same Pandoc header and rewrites internal `.md` links to `.html`; raw `.md` copies are kept for compatibility. **`python_examples_pages_links.py`** applies the same rules to the pdoc examples page.
