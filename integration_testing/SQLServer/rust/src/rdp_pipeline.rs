...
fn normalize_mssql_url(url: &str) -> Result<String, String> {
...
pub fn import_csv_mssql(
    ...
    let table = table_spec["connectors"]["mssql"]["table"]
        .as_str()
        .ok_or("mssql table missing in table spec")?;
    let url = normalize_mssql_url(connect_url)?;
    ...
    let sink = inter["sink_results"]
        .as_array()
        .and_then(|arr| arr.iter().find(|s| s["kind"] == "mssql"))
        .ok_or("missing mssql sink result")?;
    if sink["status"] != "ok" {
        return Err(format!("mssql sink failed: {sink}"));
    }
    let loaded = sink["row_count"].as_u64().ok_or("missing row_count")? as usize;
    Ok((ingested, loaded))
}

pub fn verify_count_mssql(connect_url: &str, expected: usize) -> Result<usize, String> {
    ...
    let table = table_spec["connectors"]["mssql"]["table"]
        .as_str()
        .ok_or("mssql table missing")?;
    let url = normalize_mssql_url(connect_url)?;
    let schema = Schema::new(vec![Field::new("cnt", DataType::Int64)]);
    let query = format!("SELECT CAST(COUNT(*) AS BIGINT) AS cnt FROM {table}");
