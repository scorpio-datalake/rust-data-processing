"""Databricks integration: RDP s3 warehouse → Spark SQL table + COUNT verify."""
...
def _require_integration() -> None:
    if os.environ.get("RUN_DATABRICKS_INTEGRATION") != "1":
        pytest.skip("set RUN_DATABRICKS_INTEGRATION=1 (via Databricks/run_databricks_tests.py)")
...
def test_python_databricks_sql_roundtrip() -> None:
    _require_integration()
    max_rows = int(os.environ.get("INTEG_MAX_IMPORT_ROWS", "500"))
    inter = import_csv_platform_pipeline(
        connector="databricks", csv_path=_uber_csv(), max_rows=max_rows
    )
    expected = int(inter["ingested_row_count"])
    assert expected > 0
    sink = next(s for s in inter["sink_results"] if s["kind"] == "databricks")
    assert sink["status"] == "ok"
    assert sink["row_count"] == expected
    assert str(sink.get("table_uri", "")).startswith("s3://")
    verify_platform_sql("databricks", expected)
