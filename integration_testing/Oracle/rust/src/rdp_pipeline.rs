...
pub fn import_csv_oracle(
    ...
    let table = table_spec["connectors"]["oracle"]["table"]
        .as_str()
        .ok_or("oracle table missing in table spec")?;
    ...
    let sink = inter["sink_results"]
        .as_array()
        .and_then(|arr| arr.iter().find(|s| s["kind"] == "oracle"))
        .ok_or("missing oracle sink result")?;
    if sink["status"] != "ok" {
        return Err(format!("oracle sink failed: {sink}"));
    }
    let loaded = sink["row_count"].as_u64().ok_or("missing row_count")? as usize;
    Ok((ingested, loaded))
}

pub fn verify_count_oracle(connect_url: &str, expected: usize) -> Result<usize, String> {
    ...
    let table = table_spec["connectors"]["oracle"]["table"]
        .as_str()
        .ok_or("oracle table missing")?;
    let schema = Schema::new(vec![Field::new("CNT", DataType::Int64)]);
    let query = format!("SELECT CAST(COUNT(*) AS NUMBER(19)) AS CNT FROM {table}");
    let ds = ingest_from_db(connect_url, &query, &schema, &IngestionOptions::default())
...
// Keep lib loaded until return (lib dropped at end of run_pipeline - symbols must not outlive)
// libloading Library is dropped after run_pipeline returns - OK since we don't retain symbols.
