"""PostgreSQL integration: RDP pipeline CSV → PostgreSQL sink → RDP verify (no psycopg)."""
...
from postgresql_common import strip_url_query_for_libpq  # noqa: E402
...
def _require_integration() -> None:
    if os.environ.get("RUN_POSTGRESQL_INTEGRATION") != "1":
        pytest.skip("set RUN_POSTGRESQL_INTEGRATION=1 (via PostgreSQL/run_tests.py)")
...
def _verify_count_rdp(url: str, expected: int) -> int:
    table = connector_table("postgresql")
    schema = [
        {"name": f["name"], "data_type": f["data_type"].lower()}
        for f in count_verify_schema("postgresql")["fields"]
    ]
    ds = rdp.ingest_from_db(
        strip_url_query_for_libpq(url),
        count_verify_query("postgresql", table),
        schema,
        {},
    )
    rows = ds.to_rows()
...
def test_python_postgresql_import_roundtrip() -> None:
    _require_integration()
    url = strip_url_query_for_libpq(os.environ["POSTGRES_CONNECT_URL"])
    csv_path = _uber_csv()
    ...
    sink = next(s for s in inter["sink_results"] if s["kind"] == "postgresql")
