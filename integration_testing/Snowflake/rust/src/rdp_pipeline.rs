//! Snowflake S3 stage pipeline + object_store read-back verify.
...
fn run_pipeline(payload: Value) -> Result<Value, String> {
    ...
    unsafe {
        ...
        if out.ptr.is_null() {
            return Err("rdp_run_pipeline_json returned null".into());
        }
...
fn snowflake_stage_parquet_uri() -> Result<String, String> {
    let stage = snowflake_stage_uri()?;
    Ok(format!("{stage}load.parquet"))
}

pub fn import_csv_snowflake(csv: &Path, max_rows: usize) -> Result<(usize, usize), String> {
    ...
    let sf = &table_spec["connectors"]["snowflake"];
    let payload = json!({
    ...
    let sink = inter["sink_results"]
        ...
        .ok_or("missing snowflake sink")?;
    if sink["status"] != "ok" {
        return Err(format!("snowflake sink failed: {sink}"));
    }
    let loaded = sink["row_count"].as_u64().ok_or("missing row_count")? as usize;
    Ok((ingested, loaded))
}

pub fn verify_snowflake_sql(expected: usize) -> Result<usize, String> {
...
pub fn verify_stage_parquet(expected: usize) -> Result<usize, String> {
    let uri = snowflake_stage_parquet_uri()?;
