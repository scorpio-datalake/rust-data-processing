...
/**
 * Phase 1 / Phase 2 parity is tracked in {@code Planning/PHASE3_EPICS.md}. Rust exports for each
 * area will use stable symbol names; until {@code rdp_jvm_sys} exposes them, tests <strong>skip
 * </strong> (not fail). When an export appears, the corresponding test starts exercising it — extend
 * the method body with real downcalls.
 *
 * <p>Naming convention: {@code rdp_parity_<area>} (document in ADR / manifest when adding Rust
 * exports).
 */
final class ParityMatrixDeferredExportTest {
  ...
  @DisplayName("deferred FFI export present")
  void deferredExportPresent(String mangledSymbol, String label) throws Throwable {
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      boolean present = lookup.find(mangledSymbol).isPresent();
